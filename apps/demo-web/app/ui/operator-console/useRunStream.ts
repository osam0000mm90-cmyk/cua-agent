...
import {
  ...
  extractAssistantReplyFromRunDetail,
...
import { loadStoredTranscript, saveStoredTranscript } from "./chat-memory";
...
export function useRunStream({
  ...
  const [capabilityManifest, setCapabilityManifest] = useState<AgentCapabilityManifest | null>(null);
  ...
  function appendManualTranscript(entry: TranscriptEntry) {
    setManualTranscript((current) => [
      ...filterVisibleTranscriptEntries(current).slice(-23),
      entry,
    ]);
  }

  function appendAssistantTranscriptFromRun(detail: RunDetail) {
    const reply = extractAssistantReplyFromRunDetail(detail);

    if (!reply) {
      return;
    }

    const key = `assistant-run-${detail.run.id}`;
    const createdAt = detail.run.completedAt ?? detail.run.startedAt;

    setManualTranscript((current) => {
      const visible = filterVisibleTranscriptEntries(current);
      const nextEntry = {
        body: reply,
        createdAt,
        key,
      ...
      const existingIndex = visible.findIndex((entry) => entry.key === key);
      ...
      return [...visible, nextEntry].slice(-24);
    });
  }
  ...
  useEffect(() => {
    ...
    async function loadCapabilityManifest() {
      ...
      try {
        const response = await fetch(`${runnerBaseUrl}/api/capabilities`, {
          cache: "no-store",
        });
        ...
        const manifest = agentCapabilityManifestSchema.parse(await response.json());

        if (!cancelled) {
          setCapabilityManifest(manifest);
        }
      } catch {
        if (!cancelled) {
          setCapabilityManifest(null);
    ...
    void loadCapabilityManifest();

    return () => {
      cancelled = true;
    };
  }, [runnerBaseUrl, runnerOnline]);
  ...
  useEffect(() => {
    ...
    setManualTranscript(
      filterVisibleTranscriptEntries(loadStoredTranscript(selectedScenario.id)),
    );
  }, [selectedScenario?.id]);

  useEffect(() => {
    if (!selectedScenario || !transcriptReady) {
      return;
    }

    saveStoredTranscript(
      selectedScenario.id,
      filterVisibleTranscriptEntries(manualTranscript),
    );
  }, [manualTranscript, selectedScenario?.id, transcriptReady]);
        ...
        if (
          ...
          void fetchRunDetail(event.runId)
            .then((detail) => {
              setActiveRun(detail);
              setRunEvents(detail.events);
              appendAssistantTranscriptFromRun(detail);
            })
            .catch(() => undefined)
            .finally(() => {
  ...
  const handleScenarioChange = (scenarioId: string) => {
    ...
    setManualLogs([]);
    setManualTranscript(nextScenario ? filterVisibleTranscriptEntries(loadStoredTranscript(nextScenario.id)) : []);
  ...
  const handleStartRun = async () => {
    ...
    const promptBundle = buildChatPromptBundle({
    ...
    setManualTranscript((current) => [
      ...filterVisibleTranscriptEntries(current).slice(-23),
      createManualTranscript("operator", "User", prompt),
    ]);
    setRunEvents([]);
    setActionIssue(null);
    closeEventStream();
  ...
  const handleResetWorkspace = async () => {
    ...
    try {
      ...
      if (state.cancelledRunId) {
        ...
        appendAssistantTranscriptFromRun(cancelledDetail);
