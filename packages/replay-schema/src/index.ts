...
export const labIdSchema = z.enum(["kanban", "paint", "booking", "desktop"]);
export type LabId = z.infer<typeof labIdSchema>;

export const categorySchema = z.enum(["productivity", "creativity", "commerce", "automation"]);
...
export const agentCapabilitySchema = z.object({
  category: z.enum(["browser", "terminal", "filesystem", "audio", "desktop", "memory", "integration"]),
  description: z.string().min(1),
  name: z.string().min(1),
...
export const agentTerminalPolicySchema = z.object({
  allowDangerousCommands: z.boolean(),
  blockedPatterns: z.array(z.string().min(1)),
  mode: z.enum(["guarded", "relaxed"]),
});
export type AgentTerminalPolicy = z.infer<typeof agentTerminalPolicySchema>;

export const agentCapabilityManifestSchema = z.object({
  capabilities: z.array(agentCapabilitySchema),
  generatedAt: z.string().datetime(),
  terminalPolicy: agentTerminalPolicySchema,
  title: z.string().min(1),
