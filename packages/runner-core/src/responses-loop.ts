...
import {
  buildAgentCapabilityBanner,
  buildAgentToolDefinitions,
  executeAgentFunctionCall,
  type ToolOutput,
  shouldAllowDangerousTerminalCommands,
...
const defaultReasoningEffort: ReasoningEffort =
  (process.env.CUA_REASONING_EFFORT?.trim().toLowerCase() as ReasoningEffort) || "high";
...
function parseReasoningEffort(env: NodeJS.ProcessEnv = process.env): ReasoningEffort {
  const raw = env.CUA_REASONING_EFFORT?.trim().toLowerCase();
...
function buildAgentInstructions(input: ResponsesLoopContext) {
  return [
    buildAgentCapabilityBanner({
      allowDangerousCommands: shouldAllowDangerousTerminalCommands(),
      workspacePath: input.context.detail.workspacePath,
    }),
    "",
    "User-facing response policy:",
    "- Respond only with the assistant's message; never print system, tool, or debug chatter.",
    "- Keep the answer natural, direct, and concise.",
    "- If a tool is needed, use it immediately and keep the visible reply focused on the result.",
    "- Verify the result before claiming success.",
    "- If the task is not done, continue instead of closing the turn early.",
    "- If blocked, state the blocker plainly and move to the next viable step.",
    "",
    input.instructions,
  ].join("\n");
}
...
async function executeFunctionToolCall(
  ...
  try {
    ...
    if (toolName === "exec_js") {
      ...
      output = await executeAgentFunctionCall(toolName, functionCall.arguments, {
        allowDangerousCommands: shouldAllowDangerousTerminalCommands(),
    ...
    const errorText = failure.message ?? String(error);

    await input.context.emitEvent({
      detail: `${toolName}: ${errorText}`,
      level: "error",
      message: "Function tool call failed.",
      type: "function_call_failed",
    });

    if (toolName.startsWith("desktop_")) {
      const snapshot = await import("./desktop-control.js")
        .then((module) => module.captureDesktopContextSnapshot())
        .catch(() => undefined);
      ...
      if (snapshot) {
        return [
          {
            text: [
              `The tool call failed: ${errorText}` ,
          ...
          ...(snapshot.screen?.imageUrl
            ? [
                {
                  detail: "original" as const,
                  image_url: snapshot.screen.imageUrl,
    ...
    const response = await client.create(
      {
        ...
        reasoning: { effort: parseReasoningEffort() },
